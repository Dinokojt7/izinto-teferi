...
class _InboxViewState extends State<InboxView> {
  ...
  // Simplified notification setup using only Firebase Messaging
  ...
  // Show foreground notification using SnackBar
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(
          label: 'View',
          onPressed: () {
            // Navigate to inbox or specific message
          },
        ),
      ),
    );
        ...
        body: jsonEncode(
          <String, dynamic>{
            ...
            'notification': <String, dynamic>{
              'body': message,
              'title': 'New Message - Order $orderId',
              'android_channel_id': 'izinto_chat'
            },
            'to': token,
          },
        ),
    ...
    try {
      ...
      if (documentSnapshot.exists) {
        final updatedChatRoom =
            List.from(documentSnapshot.data()!['chat room'] ?? []);
        final newMapItem = {
    ...
    FirebaseFirestore.instance
      ...
      if (mounted && userData.exists) {
        setState(() {
          ...
          _orderDate = userData['createdAt']?.toString() ?? '';
        });
      }
    });
  }
      ...
      return StreamBuilder<QuerySnapshot>(
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.active) {
            return _isLoading ? _buildLoadingView() : _buildInboxView(user);
          }
  ...
  Widget _buildInboxView(UserModel user) {
    return _size! < 1 ? _buildEmptyInboxView() : _buildChatView(user);
  }

  Widget _buildEmptyInboxView() {
    return Scaffold(
      body: SafeArea(
        child: Container(
          width: double.maxFinite,
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: Dimensions.width20 * 2),
                      Image(
  ...
  Widget _buildChatView(UserModel user) {
    return Scaffold(
      body: SafeArea(
        child: Column(
          children: [
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  String? messageText = message.values.first[0];
                  final messageTime = message.values.first[1];
                  final isUserMessage = message.keys.first == user.uid;
                      ...
                      Align(
                        ...
                        child: Container(
                          ...
                          padding: const EdgeInsets.only(
                              left: 5.0, top: 3, bottom: 3, right: 16),
                          decoration: BoxDecoration(
                            color: isUserMessage
                                ? Color(0xff9A9484).withOpacity(0.7)
                                : Colors.white,
                            borderRadius:
                                BorderRadius.circular(Dimensions.radius15),
                            border: Border.all(
                                width: 0.1, color: Color(0xff966C3B)),
                          ),
                          child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      border: Border.all(
                          width: 1, color: Color(0xff9A9484).withOpacity(0.7)),
                      borderRadius: BorderRadius.circular(Dimensions.radius15),
                      color: Colors.green,
                    ),
                    child: Text(
              ...
              GestureDetector(
                onTap: () => _launchPhoneDialer('$contact'),
                child: Container(
  ...
  Widget _buildMessageInput() {
    return Container(
      margin: const EdgeInsets.only(left: 8.0, right: 3.0, top: 8.0),
      decoration: BoxDecoration(color: Color(0xff9A9484).withOpacity(0.07)),
      child: Row(
        children: [
          Expanded(
            child: TextField(
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              if (message.isNotEmpty) {
                ...
                sendOrderNotification(deviceToken!, message);
                updateChatRoom(message, messageTime);
